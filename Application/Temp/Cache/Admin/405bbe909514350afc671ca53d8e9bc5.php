...
<head>
    ...
    <title>JUNGO会员管理系统后台 - 系统日志</title>
...
<body class="gray-bg">
            ...
            <div class="col-sm-12">
                <div class="ibox float-e-margins">
                    ...
                    <div class="ibox-content">
                        <div class="">
                            <a onclick="logDel();" href="javascript:void(0);" class="btn btn-warning "><i class="fa fa-trash"></i> &nbsp;删除选中日志</a>
                            <a onclick="logClear();" href="javascript:void(0);" class="btn btn-danger "><i class="fa fa-trash"></i> &nbsp;清空日志</a>
                        </div>
                        ...
                        <table class="table table-striped table-bordered table-hover dataTables-example" id="dataTable">
                            ...
                            <tbody>
                                ...
                                <?php if(is_array($logInfo)): $i = 0; $__LIST__ = $logInfo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$L): $mod = ($i % 2 );++$i;?><tr class="gradeA">
                                    <td><input type="checkbox" value="<?php echo ($L["id"]); ?>" name="check[]"></td>
                                    <td><?php echo ($L["id"]); ?></td>       
                                    <td><?php echo ($L["usr"]); ?></td>
                                    <td><?php echo ($L["module_name"]); ?></td>  
                                    <td><?php echo ($L["action_name"]); ?></td>
                                    <td><?php echo ($L["info"]); ?></td>
                                    <td><?php echo (date('Y-m-d H:i:s',$L["logtime"])); ?></td>
    ...
    function logDel () {
        layer.confirm('是否要批量删除这些选中记录？', {icon: 3, title:'删除确认'}, function(index){
            $("#delsForm").ajaxSubmit({
              type: "post",
              url: "<?php echo U('Sys/logsdel');?>",
              dataType: "json",
    ...
    function logClear () {
        layer.confirm('是否要清空所有记录？', {icon: 3, title:'删除确认'}, function(index){
            $("#delsForm").ajaxSubmit({
              type: "post",
              url: "<?php echo U('Sys/logclear');?>",
              dataType: "json",
              success: function(result){
                 var getStatu=result.status;
                 if (getStatu==0) 
