                      ...
                      <table class="table table-striped table-bordered table-hover dataTables-example" id="dataTable">
                          ...
                          <tbody>
                              ...
                              <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$P): $mod = ($i % 2 );++$i;?><tr class="gradeA">
                                  <td><?php echo ($P["ID"]); ?></td> 
                                  <td><?php echo (getmebinfo($P["fromid"],'mname')); ?></td>
                                  <td><?php echo (getmebinfo($P["fromid"],'truename')); ?></td>

                                  <td><?php echo (getmebinfo($P["fromid"],'moblie')); ?></td>
                                  <td><?php echo (getmebinfo($P["toid"],'mname')); ?></td>
                                  <td><?php echo (getmebinfo($P["toid"],'moblie')); ?></td>
                                  <?php $fromLv = $P['levelto'] - 1 ; ?>
                                  <td style="text-align:center;"><?php echo ($fromLv); ?>→<?php echo ($P["levelto"]); ?></td>  
                                  <td><?php echo (date("Y-m-d H:i:s",$P["fromtime"])); ?></td>  
                                  <td><?php if(($P["status"]) == "0"): ?><span class="label label-info">已升级</span><?php else: ?><span class="label label-success">待升级</span><?php endif; ?></td>
